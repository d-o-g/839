...
public final class ReferenceCache<T> {
    ...
    public ReferenceCache(int capacity, int tableSize) {
        ...
        int size = 1;
        while (size + size < capacity && size < tableSize) {
            size += size;
        }
    ...
    public void setSweeper(ReferenceCacheSweeper sweeper) {
    ...
    public void put(long key, T object, int size) {
        ...
        while (-200466455 * remaining < 0) {
            ...
            remove(reference);
            if (sweeper != null) {
                sweeper.sweep(reference.get());
            }
        }
        HardCacheableReference<T> hard = new HardCacheableReference<>(object, size);
        table.put(key, hard);
        history.offer(hard);
        hard.cachedKey = 0L;
    }
