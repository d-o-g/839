...
public class Class341 implements Interface43 {
    ...
    public static int method5864(int i, int i_0_, int i_1_, boolean bool, int i_2_) {
        ...
        Client.printToConsole(new StringBuilder().append("Stream - Input: ")
    ...
    static String method5865(Component class226) {
        if (Client.method17050(class226).method15633() == 0) {
            return null;
        }
        if (class226.selectedAction == null || class226.selectedAction.trim().length() == 0) {
            if (Client.aBool10603) {
                return "Hidden-use";
            }
            return null;
        }
        return class226.selectedAction;
    }
    ...
    static final void method5866(int i) {
        for (SceneMod sceneMod = SceneMod.pendingSpawns.head(); null != sceneMod; sceneMod = SceneMod.pendingSpawns.next()) {
            Class604.pushSceneMod(sceneMod, false);
        }
        for (SceneMod sceneMod = SceneMod.pendingRotations.head(); null != sceneMod; sceneMod = SceneMod.pendingRotations.next()) {
            Class604.pushSceneMod(sceneMod, true);
        }
    }
