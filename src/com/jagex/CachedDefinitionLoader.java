...
public class CachedDefinitionLoader<D extends Definition> implements DefinitionLoader<D> {
    ...
    CachedDefinitionLoader(GameContext context, Language language, Archive archive, Js5ConfigGroup config, int capacity,
    ...
    static final void method13913(ScriptExecutionContext class613, int i) throws ScriptException {
        ...
        Client.camera.method4821(Vector3f.create(f, f_5_, f_6_), f_7_, 1476968774);
    }
    ...
    D load(int id) {
        ...
        synchronized (archive) {
            data = Js5ConfigGroup.get(archive, config, id);
        }
