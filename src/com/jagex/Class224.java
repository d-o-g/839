...
public class Class224 implements Definition {
    ...
    public boolean method4205(VariableLoader loader, VariableProvider provider) {
        ...
        if (varp * -1607573655 != -1) {
            Varp parameter = loader.getVarp(VarpType.PLAYER, -1607573655 * varp);
            index = provider.getVarpValue(parameter);
        } else if (-1 != -312709937 * varbit) {
            VarpBit variable = loader.getVarpBit(varbit * -312709937);
        ...
        if (-1538616479 * anInt2322 != -1) {
            Varp variable = loader.getVarp(VarpType.PLAYER, anInt2322 * -1538616479);
            value = provider.getVarpValue(variable);
        } else if (anInt2321 * 447716099 != -1) {
            VarpBit variable = loader.getVarpBit(anInt2321 * 447716099);
            value = provider.getVarpBitValue(variable);
        } else {
            return true;
        }
    ...
    public final Class224 method4211(VariableLoader loader, VariableProvider provider) {
        ...
        if (-312709937 * varbit != -1) {
            VarpBit variable = loader.getVarpBit(-312709937 * varbit);
            if (variable != null) {
                index = provider.getVarpBitValue(variable);
            }
        } else if (-1 != -1607573655 * varp) {
            Varp variable = loader.getVarp(VarpType.PLAYER, -1607573655 * varp);
            if (variable != null) {
