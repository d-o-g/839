...
public class ConnectionInfo {
    ...
    public static void method13843(int i, int i_1_, int i_2_, String option) {
        Component component = Class185.method3584(i_1_, i_2_);
        if (null != component) {
            if (null != component.anObjectArray2553) {
                ScriptContext script = new ScriptContext();
                script.source = component;
                script.menuIndex = i * -718980677;
                script.menuOption = option;
                script.arguments = component.anObjectArray2553;
                Script.execute(script);
            }

            if (Client.method17050(component).method15632(i - 1)) {
                BufferedConnectionContext connection = Client.getActiveConnection();
                if (Client.connectionState * -1501875989 == 8 || -1501875989 * Client.connectionState == 16
                        || -1501875989 * Client.connectionState == 6) {
                    if (component.aString2530 != null && null != component.aClass227_2531) {
                        OutgoingFrame frame = OutgoingFrame.prepare(OutgoingFrameMeta.aClass366_3946,
                                connection.encryptor);
                        frame.payload.writeByte(7 + Buffer.encodedStringLength(component.aString2530) + 1);

                        frame.payload.writeShortA(i_2_);
                        frame.payload.writeByte(i);
                        frame.payload.writeInt(i_1_);
                        frame.payload.writeByte(component.aClass227_2531.getId());
                        frame.payload.writeCString(component.aString2530);

                        connection.sendFrame(frame);
                    } else {
                        Class222.method4166(connection, component, i, i_1_, i_2_);
                    }
                }
            }
        }
