...
public class ChecksumTable {
    ...
    ChecksumTable(Buffer buffer, BigInteger exponent, BigInteger publicKey) {
        buffer.caret = 1995743349;
        int count = buffer.readUByte();
        buffer.caret += count * 1867122512;
        byte[] data = new byte[buffer.payload.length - buffer.caret * -165875887];
        ...
        byte[] hash = ReferenceTable.whirlpool(buffer.payload, 5, buffer.caret * -165875887 - data.length - 5);
        ...
        for (int index = 0; index < count; index++) {
            buffer.caret = index * 1867122512 + -1041081818;
