...
public abstract class Class222 implements DefinitionProvider<Class224> {
    ...
    static void method4166(BufferedConnectionContext connection, Component component, int type, int i_2_, int i_3_) {
        if (type == 1) {
            OutgoingFrame frame = OutgoingFrame.prepare(OutgoingFrameMeta.aClass366_4041, connection.encryptor);
            OutgoingFrame.method9168(frame, i_2_, i_3_, component.itemId * 1391525437);
            connection.sendFrame(frame);
        } else if (type == 2) {
            OutgoingFrame frame = OutgoingFrame.prepare(OutgoingFrameMeta.aClass366_3980, connection.encryptor);
            OutgoingFrame.method9168(frame, i_2_, i_3_, 1391525437 * component.itemId);
            connection.sendFrame(frame);
        } else if (type == 3) {
