...
public class CacheableEntity extends Entity {
    ...
    public static final int method15590(long serverSeed, String string) {
        ...
        encrypted.caret = 1258142129 * length;
        encrypted.encodeXtea(seed);
        Buffer buffer = new Buffer(-165875887 * header.caret + 5 + -165875887 * buffer_1_.caret + -165875887
                * encrypted.caret);

        buffer.writeByte(2);
        buffer.writeByte(-165875887 * header.caret);
        buffer.writeBytes(header.payload, 0, header.caret * -165875887);
        buffer.writeByte(buffer_1_.caret * -165875887);
        buffer.writeBytes(buffer_1_.payload, 0, -165875887 * buffer_1_.caret);
        buffer.writeShort(-165875887 * encrypted.caret);
        buffer.writeBytes(encrypted.payload, 0, encrypted.caret * -165875887);
        ...
        try {
            ...
            while (true) {
                int in = input.read(buffer.payload, -165875887 * buffer.caret, 1000 - -165875887 * buffer.caret);
                if (-1 == in) {
                    break;
                }

                buffer.caret += 1258142129 * in;
                if (-165875887 * buffer.caret >= 1000) {
                    return 5;
                }
            }
            ...
            while (-165875887 * buffer.caret > 0 && 0 == buffer.payload[-165875887 * buffer.caret - 1]) {
                buffer.caret -= 1258142129;
            }

            response = new String(buffer.payload, 0, -165875887 * buffer.caret);
