...
public class OpenGlPointLighter extends Node {
    OpenGlToolkit toolkit;
    OpenGlSceneGraphLevel ground;
    PointLight light;
    int startZ;
    ...
    OpenGlPointLighter(OpenGlToolkit toolkit, OpenGlSceneGraphLevel ground, PointLight light, int[] is) {
        ...
        if (indexCount > 0) {
            ...
            indices = toolkit.createElementArrayBuffer(indexData.payload, indexData.caret * -165875887, false);
            anInterface11_10016 = toolkit.createArrayBuffer(16, data.payload, data.caret * -165875887, false);
    ...
    void update(int lightIndexX, int lightIndexZ, int tileX, int tileZ, int dx, int dz) {
        ...
        int height = ground.averageHeightAt(x, z);

        if ((dx & 0x7f) == 0 || (dz & 0x7f) == 0) {
            key = (z & 0xffffL) << 16 | x & 0xffffL;
            ShortNode linkable = (ShortNode) indexTable.get(key);
            if (linkable != null) {
        ...
        if (key != -1L) {
            indexTable.put(key, new ShortNode(index));
        }
