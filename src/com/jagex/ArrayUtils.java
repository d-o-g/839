...
public final class ArrayUtils {
    ...
    static void sort(long[] comparators, Object[] elems, int start, int end) {
        if (start < end) {
            int mid = (end + start) / 2;
            int current = start;
            long avg = comparators[mid];

            comparators[mid] = comparators[end];
            comparators[end] = avg;
            Object object = elems[mid];
            elems[mid] = elems[end];
            elems[end] = object;

            int mask = avg == Long.MAX_VALUE ? 0 : 1;
            for (int i = start; i < end; i++) {
                if (comparators[i] < avg + (i & mask)) {
                    long comparator = comparators[i];
                    comparators[i] = comparators[current];
                    comparators[current] = comparator;
                    Object elem = elems[i];
                    elems[i] = elems[current];
                    elems[current++] = elem;
                }
            }

            comparators[end] = comparators[current];
            comparators[current] = avg;
            elems[end] = elems[current];
            elems[current] = object;
            sort(comparators, elems, start, current - 1);
            sort(comparators, elems, current + 1, end);
        }
    }

    public static void sort(long[] comparators, Object[] elems) {
        sort(comparators, elems, 0, comparators.length - 1);
    }
