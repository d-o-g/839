    ...
    AsynchronousStreamRequest(ParallelHttpProvider provider, InputStream input, AsynchronousRequest request, URL url) {
        ...
        if (input != null) {
            ...
            while (in >= 0) {
                ...
                if (in > 0) {
                    if (-165875887 * buffer.caret + in >= buffer.payload.length) {
                        int i_1_ = buffer.payload.length + 10240;
                        byte[] tmp = ByteArrayPool.get(i_1_, true);
                        System.arraycopy(buffer.payload, 0, tmp, 0, buffer.caret * -165875887);
            ...
            byte[] data = new byte[-165875887 * buffer.caret];
            System.arraycopy(buffer.payload, 0, data, 0, -165875887 * buffer.caret);
            buffer.cache();
