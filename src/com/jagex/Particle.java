...
public class Particle extends PointEntity {
    ...
    Particle(ParticleProducer emitter, int i, int i_66_, int i_67_, int i_68_, int i_69_, int i_70_, int i_71_,
        ...
        super.y = i_67_ << 12;
        super.colour = i_73_;
        this.lifespan = this.lifetime = (short) i_72_;
        super.size = i_74_;
        super.rotation = (short) i_75_;
        this.aShort11656 = (short) i_76_;
        super.texture = i_77_;
        super.render = bool_78_;
        this.aShort11652 = (short) i_68_;
        this.aShort11654 = (short) i_69_;
        this.aShort11655 = (short) i_70_;
        this.speed = i_71_;
        link();
    }
            ...
            if (definition.anIntArray3758 != null) {
                if (definition.anIntArray3777 == null) {
                    ...
                    for (int i_55_ = 0; i_55_ < definition.anIntArray3758.length; i_55_++) {
                        PlayerMapDot.anInterface44_2095.load(definition.anIntArray3758[i_55_]);
    ...
    void method18122(SceneGraph sceneGraph, Toolkit toolkit, long l) {
        int tileX = x >> 12 + sceneGraph.tileScale * -1911343043;
        int tileY = y >> 12 + sceneGraph.tileScale * -1911343043;
        int tileZ = z >> 12;

        if (tileZ > 262144 || tileZ < -262144 || tileX < 0 || tileX >= sceneGraph.width * 173827901 || tileY < 0
                || tileY >= sceneGraph.length * 134933557) {
            kill();
        } else {
            ParticleSystem parent = emitter.parent;
            ParticleProducerDefinition definition = emitter.definition;
            SceneGraphLevel[] ground = sceneGraph.levels1;
            int plane = parent.plane;
            SceneGraphTile sceneGraphTile = sceneGraph.tiles[parent.plane][tileX][tileY];

            if (sceneGraphTile != null) {
                plane = sceneGraphTile.level;
            }

            int groundHeight = ground[plane].getTileHeight(tileX, tileY);
            int nextGroundHeight;
            if (plane < sceneGraph.levelCount * 453886331 - 1) {
                nextGroundHeight = ground[plane + 1].getTileHeight(tileX, tileY);
            } else {
                nextGroundHeight = groundHeight - (8 << sceneGraph.tileScale * -1911343043);
            }
            ...
            int i_62_;
            for (i_62_ = sceneGraph.levelCount * 453886331 - 1; i_62_ > 0
            ...
            if (definition.aBool3752 && i_62_ == 0 && tileZ > ground[0].getTileHeight(tileX, tileY)) {
                kill();
            } else if (i_62_ == sceneGraph.levelCount * 453886331 - 1
                    && ground[i_62_].getTileHeight(tileX, tileY) - tileZ > 8 << sceneGraph.tileScale * -1911343043) {
                kill();
            } else {
                sceneGraphTile = sceneGraph.tiles[i_62_][tileX][tileY];
                if (sceneGraphTile == null) {
                    if (i_62_ == 0 || sceneGraph.tiles[0][tileX][tileY] == null) {
                        sceneGraphTile = sceneGraph.tiles[0][tileX][tileY] = new SceneGraphTile(0);
                    }
                    boolean bool = sceneGraph.tiles[0][tileX][tileY].base != null;
                    if (i_62_ == 3 && bool) {
                        i_62_--;
                    }
                    for (int i_63_ = 1; i_63_ <= i_62_; i_63_++) {
                        if (sceneGraph.tiles[i_63_][tileX][tileY] == null) {
                            sceneGraphTile = sceneGraph.tiles[i_63_][tileX][tileY] = new SceneGraphTile(i_63_);
                            if (bool) {
                ...
                if (definition.aBool3778) {
                    int i_64_ = x >> 12;
                    int i_65_ = y >> 12;
                    if (sceneGraphTile.boundary != null) {
                        Class498 class498 = sceneGraphTile.boundary.method16293(toolkit);
                        if (class498 != null && class498.method8681(i_64_, tileZ, i_65_)) {
                            kill();
                            return;
                        }
                    }
                    if (sceneGraphTile.linkedBoundary != null) {
                        Class498 class498 = sceneGraphTile.linkedBoundary.method16293(toolkit);
                        if (class498 != null && class498.method8681(i_64_, tileZ, i_65_)) {
                            kill();
                            return;
                        }
                    }
                    if (sceneGraphTile.decoration != null) {
                        Class498 class498 = sceneGraphTile.decoration.method16293(toolkit);
                        if (class498 != null && class498.method8681(i_64_, tileZ, i_65_)) {
                            kill();
                            return;
                        }
                    ...
                    for (EntityIterator entityIterator = sceneGraphTile.entityIterator; entityIterator != null; entityIterator = entityIterator.next) {
                        Class498 class498 = entityIterator.current.method16293(toolkit);
                        if (class498 != null && class498.method8681(i_64_, tileZ, i_65_)) {
                            kill();
                            return;
                        }
                ...
                parent.entities.entities.offer(this);
            }
        }
    }
