    ...
    void pulse() {
        if (isActive()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (frame != null && frame.payload.caret * -165875887 - size >= 252 - (6 + flagsSize())) {
                    break;
                }
                ...
                if (x == 1280612997 * lastX && 1280642383 * lastY == y) {
                    ...
                    if (null == frame) {
                        ...
                        size = frame.payload.caret * -165875887;
                        frame.payload.caret += -1778683038;
                        total = 0;
                        count = 0;
                    }
            ...
            if (frame != null) {
                frame.payload.writeSizeByte(frame.payload.caret * -165875887 - size);
                int start = -165875887 * frame.payload.caret;
                frame.payload.caret = size * 1258142129;
                frame.payload.writeByte(total / count);
                frame.payload.writeByte(total % count);
                frame.payload.caret = 1258142129 * start;
