...
public class OpenGl2DTextureCache {
    ...
    OpenGl2DTextureCache(OpenGlToolkit toolkit, TextureLoader loader) {
    ...
    OpenGl2DTexture get(TextureMetrics metrics, int length) {
        ...
        if (!loaded(TextureType.TEXTURE_AND_MATERIAL, metrics, length)) {
            return null;
        }
        ...
        if (!metrics.aBool1701 || !toolkit.method2496()) {
            ...
            if (metrics.aClass554_1727 != Class554.aClass554_7499 || !Class279.method5212(metrics.aByte1719)) {
                data = loader.loadOpaque(TextureType.TEXTURE_AND_MATERIAL, metrics.id * -464558709, 0.7F);
            } else {
                data = loader.loadTranslucent(TextureType.TEXTURE_AND_MATERIAL, metrics.id * -464558709, 0.7F);
            }

            texture = new OpenGl2DTexture(toolkit, OpenGL.GL_TEXTURE_2D, length, length, metrics.mipmapLevel != 0,
                    data, 0, 0, false);
        } else {
            float[] fs = loader.loadCombined(TextureType.TEXTURE_AND_MATERIAL, metrics.id * -464558709, 0.7F);
    ...
    boolean loaded(TextureType type, TextureMetrics metrics, int length) {
        if (length == -1) {
            length = metrics.length * -931821243;
        }
