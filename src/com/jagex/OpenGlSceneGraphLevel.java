...
public class OpenGlSceneGraphLevel extends SceneGraphLevel {
    ...
    OpenGlSceneGraphLevel(OpenGlToolkit toolkit, int flags2, int flags, int width, int length, int[][] heights, int[][] lighting,
                          int units) {
    ...
    public void finish() {
        if (vertices > 0) {
            ...
            groundMaterials = new Node[materialTable.size()];
            materialTable.values(groundMaterials);
            for (Node element : groundMaterials) {
                        ...
                        for (int index = 0; index < colours.length; index++) {
                            ...
                            if (node == null) {
                                ...
                                if (toolkit.bigEndian) {
                                    ...
                                    stream.writeBEFloat(averageHeightAt(vertexX, vertexZ) + i_82_);
                                    ...
                                    stream.writeLEFloat(averageHeightAt(vertexX, vertexZ) + i_82_);
                                ...
                                table.put(key, new ShortNode(indices[index]));
                            } else {
                                indices[index] = ((ShortNode) node).value;
    ...
    public void addBlendedTile(int x, int z, int[] offsetsX, int[] is_178_, int[] offsetsY, int[] is_180_,
        ...
        for (int index = 0; index < colours.length; index++) {
            ...
            Node node;
            for (node = materialTable.get(key); node != null; node = materialTable.nextInBucket()) {
                OpenGlTileMaterial material = (OpenGlTileMaterial) node;
            ...
            if (node == null) {
                ...
                tile[index] = (OpenGlTileMaterial) node;
            }
        }
    ...
    void method14020(int i, int i_2_, int i_3_, boolean[][] bools, boolean bool, int i_4_) {
        if (groundMaterials != null) {
            ...
            if (!lighters.isEmpty()) {
                ...
                for (Node class480 = lighters.head(); class480 != null; class480 = lighters.next()) {
