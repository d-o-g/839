...
final class BigBoyVarpProviderImpl implements BigBoyVarpProvider {
    ...
    static void method781(Player player, boolean bool) {
        if (Class56.menuOptionCount * 84366055 < 406) {
            if (player == Client.localPlayer) {
                if (Client.componentSelected && 0 != (Class577.selectedComponentAttributes * -1564025309 & 0x10)) {
                    MenuItem.add(
                            Client.selectedComponentAction,
                            new StringBuilder().append(Client.aString10789).append(" ").append(Class60.arrow)
                                    .append(" ").append(Item.getColorTags(16777215))
                    ...
                    if (player.combat * 1340998367 >= player.boosted * -919525781) {
                        string = new StringBuilder()
                                ...
                                .append(bool_0_ ? Class167.getCombatDifferenceHtml(1340998367 * player.combat,
                                        1340998367 * Client.localPlayer.combat) : Item.getColorTags(16777215))
                        ...
                        string = new StringBuilder()
                                .append(player.formattedName(true))
                                .append(bool_0_ ? Class167.getCombatDifferenceHtml(1340998367 * player.combat,
                                        Client.localPlayer.combat * 1340998367) : Item.getColorTags(16777215))
                                .append(Class60.LEFT_PARENTHESIS).append(level).append(1340998367 * player.combat)
                ...
                if (Client.componentSelected && !bool && (Class577.selectedComponentAttributes * -1564025309 & 0x8) != 0) {
                    MenuItem.add(Client.selectedComponentAction,
                            new StringBuilder().append(Client.aString10789).append(" ").append(Class60.arrow)
                                    .append(" ").append(Item.getColorTags(16777215)).append(string).toString(),
                ...
                if (!bool) {
                    for (int index = 7; index >= 0; index--) {
                        if (null != Client.playerOptions[index]) {
                            ...
                            MenuItem.add(Client.playerOptions[index], Item.getColorTags(colour) + string, cursor,
                                    type, -1, player.index * 387935885, 0, 0, true, false, 387935885 * player.index,
                                    false);
                        }
                    }
                } else if (!bool_4_) {
                    MenuItem.add(new StringBuilder().append(Item.getColorTags(13421772)).append(string).toString(),
                            "", -1, -1, 0, 0L, 0, 0, false, true, player.index * 387935885, false);
                }
                if (!bool && !bool_4_) {
                    for (MenuItem option = MenuItem.options.head(); option != null; option = MenuItem.options
                            .next()) {
                        if (23 == -20536735 * option.opcode) {
                            option.aString11415 = new StringBuilder().append(Item.getColorTags(16777215))
    ...
    static final void method785(long l) {
        if (Client.scene.getGraph() != null) {
            if (2 == Client.mapState * 1212513917 || 1212513917 * Client.mapState == 4) {
                ParameterDefinition.method924(l);
            } else if (1212513917 * Client.mapState == 6) {
                ChatMessageIcon.method5968(l);
            }
        }
        ...
        for (int i = 0; i < -573888519 * Client.anInt10848; i++) {
            Client.aBoolArray10851[i] = Client.validComponents[i];
            Client.validComponents[i] = false;
        }
        Client.anInt10864 = Client.engineCycle * -401913693;
        ...
        Class611.method10321(Client.localPlayer.plane, (int) vector3f.x + i_16_, (int) vector3f.z + i_16_,
    ...
    @Override
    public VarpBitLoader method120() {
        return VarpBit.loader;
    }
