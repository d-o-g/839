...
public abstract class Class136 implements TextureLoader {
    ...
    @Override
    public float[] loadCombined(TextureType type, int id, float intensity) {
        if (type != TextureType.TEXTURE_AND_MATERIAL) {
            return null;
        }
        ...
        int[] argb = null;
        byte[] data = get(TextureType.MATERIAL, id);
    ...
    @Override
    public abstract boolean loaded(TextureType textureType, int id);

    @Override
    public int[] loadOpaque(TextureType type, int id, float intensity) {
        return loadRaw(type, id, intensity);
    }

    @Override
    public int[] loadTranslucent(TextureType type, int id, float intensity) {
    ...
    abstract byte[] get(TextureType type, int id);
    ...
    int[] loadRaw(TextureType type, int id, double intensity) {
