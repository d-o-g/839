...
public class Class42 {
    ...
    static final void method1043(int plane, int worldX, int worldZ, int i_3_, int heightOffset, boolean bool,
                                 boolean bool_5_) {
        if (!bool
                && (worldX < 512 || worldZ < 512 || worldX > (Client.scene.getMapWidth() - 2) * 512 || worldZ > (Client.scene
                .getMapLength() - 2) * 512)) {
            float[] fs = Client.aFloatArray10736;
            Client.aFloatArray10736[1] = -1.0F;
            fs[0] = -1.0F;
        } else {
            int height = SceneGraph.getAverageHeight(worldX, worldZ, plane) - heightOffset;
