    ...
    public boolean pulse() {
        ...
        try {
            ...
            for (int in = 0; in < 100; in++) {
                ...
                if (null == current) {
                    int remaining = 5 - -165875887 * header.caret;
                    if (remaining > available) {
                        remaining = available;
                    }

                    connection.read(header.payload, -165875887 * header.caret, remaining);
                    if (0 != encryptionKey) {
                        for (int index = 0; index < remaining; index++) {
                            header.payload[-165875887 * header.caret + index] ^= encryptionKey;
                        }
                    }

                    header.caret += remaining * 1258142129;
                    if (header.caret * -165875887 >= 5) {
                        header.caret = 0;
                    ...
                    if (buffer == null) {
                        int remaining = 5 - -165875887 * meta.caret;
                        if (remaining > available) {
                            remaining = available;
                        }

                        connection.read(meta.payload, meta.caret * -165875887, remaining);
                        if (0 != encryptionKey) {
                            for (int index = 0; index < remaining; index++) {
                                meta.payload[index + -165875887 * meta.caret] ^= encryptionKey;
                            }
                        }

                        meta.caret += remaining * 1258142129;
                        if (meta.caret * -165875887 >= 5) {
                            meta.caret = 0;
                        ...
                        if (length > remaining - -165875887 * buffer.caret) {
                            length = remaining - buffer.caret * -165875887;
                        }
                        ...
                        connection.read(buffer.payload, -165875887 * buffer.caret, length);
                        if (encryptionKey != 0) {
                            for (int index = 0; index < length; index++) {
                                buffer.payload[index + -165875887 * buffer.caret] ^= encryptionKey;
                            }
                        }

                        buffer.caret += length * 1258142129;
                        currentBlock += length * 1370706345;

                        if (remaining == -165875887 * buffer.caret) {
                            current.uncache();
