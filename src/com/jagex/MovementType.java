...
public class MovementType implements Identifiable {
                ...
                if (skip) {
                    ...
                    if (x < i_14_ && y < i_15_ && x + width >= 0 && y + length >= 0) {
                        if (i_26_ != -1050493863 * ObjectCategory.aClass541_7358.type
                                ...
                                || definition.type * -1893691057 != 0 || definition.anInt7367 * 383562139 == 1
        ...
        if (centralRegionX > 3 && centralRegionZ > 3 && centralRegionX < Client.scene.getMapWidth() - 4
            ...
            for (int x = centralRegionX - 4; x <= centralRegionX + 4; x++) {
                for (int z = centralRegionZ - 4; z <= centralRegionZ + 4; z++) {
                    ...
                    int heightOffset = 0;
                    byte[][] is = Client.scene.method7688(plane);
                    if (is != null) {
                        heightOffset = 8 * (is[x][z] & 0xff) << 2;
                    }

                    if (null != sceneGraph.levels1 && sceneGraph.levels1[plane] != null) {
                        int i_9_ = i_3_ - (sceneGraph.levels1[plane].getTileHeight(x, z) - heightOffset);
                        if (i_9_ > i_4_) {
                            i_4_ = i_9_;
                        }
                    }
    ...
    static int getQuestPoints(VariableProvider provider, DefinitionLoader<QuestDefinition> loader) {
        int i_11_ = 0;

        for (int id = 0; id < loader.count(); id++) {
            QuestDefinition quest = loader.get(id);

            if (quest.isCompleted(provider)) {
                i_11_ += quest.points * 527933495;
            }
        }
    ...
    static final boolean method10455(char c) {
        if (Character.isISOControl(c)) {
            return false;
        } else if (Client.isAlphaNumeric(c)) {
            return true;
        }
